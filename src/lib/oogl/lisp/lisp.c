...
LObject *LEval(LObject *obj) {
    ...
    if (list->car->type == LFUNC) {
        ...
        /**
         * ture into the function which is point out
         */
...
bool LDefun(const char *name, LObjectFunc func, const char *help) {
    ...
    if (index >= 0) {
        ...
        if (lfunction->lambda == NULL) {
            VARARRAY(builtin,
            char, strlen(name) + sizeof("-builtin--"));
...
LObject *LEvalFunc(const char *name, ...) {
    /**
     * 在C中，当我们无法列出传递函数的所有实参的类型和数目时,可以用省略号指定参数
     * void foo(...);
     * void foo(parm_list,...);
     * 函数参数是以数据结构:栈的形式存取,从右至左入栈。
     * 首先是参数的内存存放格式：参数存放在内存的堆栈段中，在执行函数的时候，从最后一个开始入栈。
     * 因此栈底高地址，栈顶低地址，举个例子如下：
     * 从理论上说，我们只要探测到任意一个变量的地址，并且知道其他变量的类型，
    ...
    LFree(obj);
    /**
     * LObject *LEval(LObject *obj) is in this file
     */
    return val;
}
...
LDEFINE(help, LVOID,
    ...
    switch (seen) {
        default:
            if (seen % 4) fprintf(outf, "\n");
            break;
        case 0:
            fprintf(outf, nomatch, pat);
            break;
        case 1:
